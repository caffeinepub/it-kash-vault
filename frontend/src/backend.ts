...
import type { Game as _Game, _CaffeineStorageRefillInformation as __CaffeineStorageRefillInformation, _CaffeineStorageRefillResult as __CaffeineStorageRefillResult } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async addGame(arg0: string, arg1: string, arg2: string, arg3: string): Promise<void> {
        if (this.processError) {
            try {
    ...
    async getGameByTitle(arg0: string): Promise<Game | null> {
        if (this.processError) {
            try {
                const result = await this.actor.getGameByTitle(arg0);
    ...
    async getMostPopularGames(arg0: bigint): Promise<Array<Game>> {
        if (this.processError) {
            try {
                const result = await this.actor.getMostPopularGames(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n8(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Game]): Game | null {
    return value.length === 0 ? null : value[0];
}
